...
export const createCustomer = (data: Omit<Customer, 'id' | 'created_at' | 'updated_at' | 'transaction_terms'>): Promise<Customer> => {
    return fetchWithAuth('/api/definitions/customers', {
        method: 'POST',
        body: JSON.stringify(data),
    });
};
export const updateCustomer = (id: number, data: Omit<Customer, 'id' | 'created_at' | 'updated_at' | 'transaction_terms'>): Promise<Customer> => {
    return fetchWithAuth(`/api/definitions/customers/${id}`, {
        method: 'PUT',
        body: JSON.stringify(data),
    });
};
